  ...
  panels_cart_to_polar <- lapply(layoutNames, function(panel) {
    ...
    from_cart_to_polar <- lapply(s2$grobs[[i]]$children[[which_bar.gTree]]$children, function(p) {
      ...
      if (nrow(polars[round(polars$phi, 6) > round(phi_to, 6), ]) > 0){
        phi_to <- phi_from
        polars <- polars %>% mutate(phi = ifelse(round(phi, 6) > round(phi_to, 6),
                                                 phi - 2*pi,
                                                 phi))
      }
      ...
      polars <- polars %>% select(r, phi)
      extrema <- polars %>% summarise_all(list(min = min, max = max), na.rm = TRUE)
      
      polar_center <- list(r = (extrema$r_min + extrema$r_max) / 2,
                           phi = (extrema$phi_min + extrema$phi_max) / 2)
    ...
    from_cart_to_polar %>% jsonlite::write_json(path = "cartesian_to_polar.json", auto_unbox = TRUE)
...
polygons_normalized <- lapply(names(poly_centers), function(i) {
  ...
  lapply(polygons, function(p) {
    p$cartesian_center$x <- p$cartesian_center$x * panel_width + panel_center["x"]
    p$cartesian_center$y <- -p$cartesian_center$y * panel_height + panel_center["y"]
    p
  })
})

names(polygons_normalized) <- names(poly_centers)
